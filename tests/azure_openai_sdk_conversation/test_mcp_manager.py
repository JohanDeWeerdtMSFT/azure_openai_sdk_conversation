...
async def test_mcp_manager_initial_and_delta(manager):
    ...
    # Delta - no state change: should still return entity map (not None)
    delta = manager.build_delta_prompt(conv_id, entities)
    assert delta is not None, "Delta must not be None; entity map must always be present"
    assert "light.test" in delta
    assert "Light" in delta
    assert "None." in delta  # no-change marker
...
async def test_delta_always_includes_entity_map_for_resolution(manager):
    ...
    delta_t2 = manager.build_delta_prompt(conv_id, entities_t2)
    ...
    delta_t3 = manager.build_delta_prompt(conv_id, entities_t3)
...
@pytest.mark.anyio
async def test_mcp_delta_returns_none_for_unknown_conversation(manager):
    """build_delta_prompt must return None for an unknown conversation ID."""
    delta = manager.build_delta_prompt("unknown_conv", [])
